...
export async function loadReceiptTemplate(): Promise<ReceiptTemplate> {
  const data = await apiClient.get<{ template: Record<string, any> }>('/settings/receipt-template');
  if (data?.template && typeof data.template === 'object') {
    const sanitized = Object.fromEntries(
      Object.entries(data.template as Record<string, any>).filter(([, value]) => value !== null && value !== undefined)
    );
...
export default function ReceiptTemplateSettings({ open, onOpenChange }: Props) {
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const data = await apiClient.postForm<{ logo_url: string }>('/settings/receipt-template/logo', payload);
            ...
            <div className="space-y-3">
              ...
              {template.showLogo && (
                <div className="flex items-center gap-4">
                  ...
                  <Button type="button" variant="outline" onClick={openLogoPicker} disabled={uploading}>
                    {uploading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />}
                    {template.logoUrl ? 'Change Logo' : 'Upload Logo'}
                  </Button>
                  <input ref={logoInputRef} id="logoUpload" type="file" accept="image/*" className="hidden" onChange={handleLogoUpload} />
                </div>
              )}
            </div>
