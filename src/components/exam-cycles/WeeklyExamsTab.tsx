...
export default function WeeklyExamsTab() {
  ...
  const [examTypeTab, setExamTypeTab] = useState('general');
  ...
  async function fetchData() {
    ...
    try {
      ...
      setExamSyllabusLinks(data.links || []);
      setClasses(data.classes || []);
      setCycles(data.cycles || []);
    } catch (error: any) {
      toast.error(error.message || 'Failed to load weekly exams data');
    } finally {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      await apiClient.post('/weekly-exams', {
        ...
        cycle_id: syllabusType === 'competitive' && formData.cycle_id ? Number(formData.cycle_id) : null,
  ...
  function openEdit(exam: WeeklyExam) {
    ...
    setFormData({
      class_id: exam.class_id, cycle_id: exam.cycle_id || '', week_number: exam.week_number?.toString() || '',
      exam_title: exam.exam_title, exam_date: exam.exam_date, exam_time: exam.exam_time,
      duration_minutes: exam.duration_minutes.toString(), total_marks: exam.total_marks.toString(),
      negative_marking: exam.negative_marking, negative_marks_value: exam.negative_marks_value.toString(),
  ...
  async function handleSaveSyllabusLinks() {
    ...
    try {
      await apiClient.put(`/weekly-exams/${selectedExam.id}/syllabus-links`, {
        syllabus_ids: selectedSyllabusIds.map((id) => Number(id)),
      });
    } catch (error: any) { toast.error(error.message); return; }
  ...
  const filteredExams = useMemo(() => {
    return exams.filter(e => {
      ...
      if (searchQuery) {
        ...
        return e.exam_title.toLowerCase().includes(q) || e.classes?.name?.toLowerCase().includes(q) || false;
      }
      return true;
    });
  }, [exams, examTypeTab, searchQuery]);
  ...
  const ExamForm = ({ onSubmit, submitLabel }: { onSubmit: (e: React.FormEvent) => void; submitLabel: string }) => (
    <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Select value={formData.cycle_id} onValueChange={v => setFormData(f => ({ ...f, cycle_id: v }))}>
              ...
              <SelectContent>
                {cycles.map(c => <SelectItem key={c.id} value={c.id}>{c.exam_type} - Cycle #{c.cycle_number} {c.is_active ? '(Active)' : ''}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="space-y-2"><Label>Exam Date *</Label><Input type="date" value={formData.exam_date} onChange={e => setFormData(f => ({ ...f, exam_date: e.target.value }))} /></div>
        <div className="space-y-2"><Label>Exam Time</Label><Input type="time" value={formData.exam_time} onChange={e => setFormData(f => ({ ...f, exam_time: e.target.value }))} /></div>
        <div className="space-y-2"><Label>Duration (min)</Label><Input type="number" value={formData.duration_minutes} onChange={e => setFormData(f => ({ ...f, duration_minutes: e.target.value }))} min="10" /></div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-2"><Label>Total Marks</Label><Input type="number" value={formData.total_marks} onChange={e => setFormData(f => ({ ...f, total_marks: e.target.value }))} min="1" /></div>
        <div className="space-y-3 pt-1">
          <div className="flex items-center gap-2"><Switch checked={formData.negative_marking} onCheckedChange={v => setFormData(f => ({ ...f, negative_marking: v }))} /><Label>Negative Marking</Label></div>
          {formData.negative_marking && <Input type="number" step="0.25" value={formData.negative_marks_value} onChange={e => setFormData(f => ({ ...f, negative_marks_value: e.target.value }))} placeholder="e.g. 0.25" />}
        </div>
      </div>
    ...
    <div className="space-y-4">
      ...
      <Tabs value={examTypeTab} onValueChange={setExamTypeTab}>
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="general" className="flex items-center gap-1.5"><BookOpen className="h-4 w-4" />General</TabsTrigger>
          <TabsTrigger value="competitive" className="flex items-center gap-1.5"><FlaskConical className="h-4 w-4" />Competitive</TabsTrigger>
        </TabsList>
      </Tabs>
      ...
      {filteredExams.length === 0 ? (
        <Card><CardContent className="py-12 text-center text-muted-foreground">No exams found. Click "Create Exam" to get started.</CardContent></Card>
      ) : (
                  ...
                  {linked.length > 0 && (
                    <div className="space-y-1">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {linked.map(s => <Badge key={s.id} variant="outline" className="text-xs">{s.subjects?.name}: {s.topic_name}</Badge>)}
                      </div>
                    </div>
                  )}
                </CardContent>
      ...
      <Dialog open={dialogOpen} onOpenChange={v => { setDialogOpen(v); if (!v) resetForm(); }}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Create {examTypeTab === 'general' ? 'General' : 'Competitive'} Weekly Exam</DialogTitle></DialogHeader>
      ...
      <Dialog open={syllabusDialogOpen} onOpenChange={v => { setSyllabusDialogOpen(v); if (!v) setSelectedExam(null); }}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Attach Syllabus Topics</DialogTitle></DialogHeader>
          {selectedExam && (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">Select syllabus topics covered in <strong>{selectedExam.exam_title}</strong></p>
              <div className="space-y-2 max-h-[50vh] overflow-y-auto">
                {getClassSyllabus(selectedExam.class_id, selectedExam.syllabus_type).length === 0 ? (
                  <p className="text-sm text-muted-foreground py-4 text-center">No syllabus topics found for this class.</p>
                ) : (
                  getClassSyllabus(selectedExam.class_id, selectedExam.syllabus_type).map(s => (
                    <label key={s.id} className="flex items-start gap-3 p-3 rounded-md border cursor-pointer hover:bg-muted/50 transition-colors">
                      <Checkbox checked={selectedSyllabusIds.includes(s.id)} onCheckedChange={() => toggleSyllabusSelection(s.id)} className="mt-0.5" />
